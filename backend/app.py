...
from database.config import WEBDAV_ENABLED, DB_TYPE
...
for key, value in os.environ.items():
    if key in ['JWT_SECRET_KEY', 'HOST', 'FLASK_PORT', 'WS_PORT', 'API_URL', 'WS_URL', 
              'DB_TYPE', 'MYSQL_HOST', 'MYSQL_PORT', 'WEBDAV_ENABLED']:
        print(f"{key}: {value}")
print("===========================\n")
...
def backup_database(current_user):
    ...
    if success:
        return jsonify({'message': '数据库备份成功'})
    else:
        return jsonify({'error': '数据库备份失败'}), 500

@app.route('/api/admin/database/webdav/config', methods=['GET'])
@token_required
@admin_required
def get_webdav_config(current_user):
    """获取 WebDAV 配置"""
    if not current_user['is_admin']:
        return jsonify({'error': '需要管理员权限'}), 403
    ...
    config = {
        ...
        'root_path': os.environ.get('WEBDAV_ROOT_PATH', '/firemail/'),
        'db_name': os.environ.get('WEBDAV_DB_NAME', 'firemail.db')
    }
    
    return jsonify(config)

@app.route('/api/admin/database/webdav/config', methods=['POST'])
@token_required
@admin_required
def save_webdav_config(current_user):
    """保存 WebDAV 配置"""
    if not current_user['is_admin']:
        return jsonify({'error': '需要管理员权限'}), 403
    ...
    try:
        ...
        # 更新或添加 WebDAV 配置
        webdav_configs = {
            'WEBDAV_ENABLED': 'true' if data.get('enabled', False) else 'false',
            'WEBDAV_URL': data.get('url', ''),
            'WEBDAV_USERNAME': data.get('username', ''),
            'WEBDAV_ROOT_PATH': data.get('root_path', '/firemail/'),
            'WEBDAV_DB_NAME': data.get('db_name', 'firemail.db')
        }
        
        # 如果提供了密码，则更新密码
        if data.get('password'):
            webdav_configs['WEBDAV_PASSWORD'] = data.get('password')
        
        # 更新环境变量
        for key, value in webdav_configs.items():
            # 更新进程内环境变量
        ...
        # 更新全局 WEBDAV_ENABLED 变量
        global WEBDAV_ENABLED
        WEBDAV_ENABLED = webdav_configs['WEBDAV_ENABLED'].lower() == 'true'
        
        # 如果启用了 WebDAV，则重新初始化 WebDAV 处理器
        if WEBDAV_ENABLED and hasattr(db, 'webdav'):
            db.webdav = None  # 清除现有处理器
            db.webdav = WebDAVHandler()  # 创建新处理器
        
        logger.info(f"管理员 {current_user['username']} 更新了 WebDAV 配置: {webdav_configs['WEBDAV_ENABLED']}")
        return jsonify({'message': 'WebDAV 配置已更新'})
    except Exception as e:
        logger.error(f"更新 WebDAV 配置失败: {str(e)}")
        return jsonify({'error': f'保存配置失败: {str(e)}'}), 500

@app.route('/api/admin/database/webdav/sync-to', methods=['POST'])
@token_required
@admin_required
def sync_to_webdav(current_user):
    """同步数据库到WebDAV"""
    if not current_user['is_admin']:
        return jsonify({'error': '需要管理员权限'}), 403
        
    if not WEBDAV_ENABLED:
        return jsonify({'error': 'WebDAV同步未启用'}), 400
        
    success = db.sync_to_webdav()
    if success:
        return jsonify({'message': '数据库已成功同步到WebDAV'})
    else:
        return jsonify({'error': '同步数据库到WebDAV失败'}), 500

@app.route('/api/admin/database/webdav/sync-from', methods=['POST'])
@token_required
@admin_required
def sync_from_webdav(current_user):
    """从WebDAV同步数据库"""
    if not current_user['is_admin']:
        return jsonify({'error': '需要管理员权限'}), 403
    ...
    success = db.sync_from_webdav()
    if success:
        return jsonify({'message': '数据库已成功从WebDAV同步'})
    else:
        return jsonify({'error': '从WebDAV同步数据库失败'}), 500
...
def get_database_info(current_user):
    ...
    info = {
        ...
        'email_count': len(db.get_all_emails())
    }
...
def parse_args():
    ...
    parser.add_argument('--port', type=int, default=int(os.environ.get('FLASK_PORT', 5000)), help='Flask服务端口')
    parser.add_argument('--ws-port', type=int, default=int(os.environ.get('WS_PORT', 8765)), help='WebSocket服务端口')
    return parser.parse_args()

def start_websocket_server():
    """启动WebSocket服务器"""
    args = parse_args()
    ws_handler.start_server(args.host, args.ws_port)
...
if __name__ == '__main__':
    ...
    print(f"WebDAV同步: {'已启用' if WEBDAV_ENABLED else '未启用'}")
